...
local000001_xls = common.get_xls("test000001.xlsx", "Card_replacement")

#申请借款页面换卡接口LB0078(借款详情页面换卡调用接口相同LB0078)
@paramunittest.parametrized(*local000001_xls)
class testCard_replacement(unittest.TestCase):
  ...
  return nowcode
    ...
    def testcard_replacement(self):
        ...
        """
        # set url
        self.url = common.get_url_from_xml('Card_replacement')
    ...
    def checkResult(self):
        ...
        if self.result == '1':
            ...
            self.assertEqual(self.info['result']["retCode"], self.retCode)
            self.assertEqual(self.info['result']["retMessage"], self.retMessage)
            print("调用接口换卡失败："+ self.retMessage)
