...
local000001_xls = common.get_xls("test000001.xlsx", "Automatic_Matching")

#自动匹配规则接口LB0100
@paramunittest.parametrized(*local000001_xls)
class testAutomatic_Matching(unittest.TestCase):
  ...
  return nowcode
    ...
    def testAutomatic_Matching(self):
        ...
        """
        # set url
        self.url = common.get_url_from_xml('Automatic_Matching')
    ...
    def checkResult(self):
        ...
        if self.result == '1':
            ...
            self.assertEqual(self.info['result']["retCode"], self.retCode)
            self.assertEqual(self.info['result']["retMessage"], self.retMessage)
            print("获取银行信息失败："+ self.retMessage)
