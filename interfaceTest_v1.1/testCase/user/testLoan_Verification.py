...
local000001_xls = common.get_xls("test000001.xlsx", "Loan_Verification")

#借款在途笔数及额度验证接口LB0082，可通过多个客户进行验证不通过案例测试，大于笔数或者额度验证不通过！
@paramunittest.parametrized(*local000001_xls)
class testLoan_Verification(unittest.TestCase):
  ...
  return nowcode
    ...
    def testLoan_Verification(self):
        ...
        """
        # set url
        self.url = common.get_url_from_xml('Loan_Verification')
    ...
    def checkResult(self):
        ...
        if self.result == '1':
            ...
            self.assertEqual(self.info['result']["retCode"], self.retCode)
            self.assertEqual(self.info['result']["retMessage"], self.retMessage)
            print("调用借款在途笔数及额度验证不通过："+ self.retMessage)
