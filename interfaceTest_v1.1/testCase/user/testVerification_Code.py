...
import hashlib #提供加密解密
import json
localReadConfig = readConfig.ReadConfig()
localConfigHttp = configHttp.ConfigHttp()
local000001_xls = common.get_xls("test000001.xlsx", "Verification_Code")
#短信验证码接口LB0109

@paramunittest.parametrized(*local000001_xls)
class testVerification_Code(unittest.TestCase):
  ...
  return nowcode
    ...
    def testerification_code(self):
        ...
        """
        # set url
        self.url = common.get_url_from_xml('Verification_Code')
    ...
    def checkResult(self):
        ...
        if self.result == '1':
            ...
            self.assertEqual(self.info['result']["retCode"], self.retCode)
            self.assertEqual(self.info['result']["retMessage"], self.retMessage)
            print("登录短信验证码验证失败："+ self.retMessage)
