...
local000001_xls = common.get_xls("test000001.xlsx", "total_debt")
#订单详情接口LB0095

@paramunittest.parametrized(*local000001_xls)
class testTotal_debt(unittest.TestCase):
  ...
  return nowcode
    ...
    def testtotal_debt(self):
        ...
        """
        # set url
        self.url = common.get_url_from_xml('total_debt')
    ...
    def checkResult(self):
        ...
        if self.result == '1':
            ...
            self.assertEqual(self.info['result']["retCode"], self.retCode)
            self.assertEqual(self.info['result']["retMessage"], self.retMessage)
            print("查看订单详情失败："+ self.retMessage)
