...
local000001_xls = common.get_xls("test000001.xlsx", "Process_authentication")

#流程认证接口LB0076
@paramunittest.parametrized(*local000001_xls)
class testProcess_authentication(unittest.TestCase):
  ...
  return nowcode
    ...
    def testProcess_authentication(self):
        ...
        """
        # set url
        self.url = common.get_url_from_xml('Process_authentication')
    ...
    def checkResult(self):
        ...
        if self.result == '1':
            ...
            self.assertEqual(self.info['result']["retCode"], self.retCode)
            self.assertEqual(self.info['result']["retMessage"], self.retMessage)
            print("调用接口流程认证失败："+ self.retMessage)
