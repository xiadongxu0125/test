...
local000001_xls = common.get_xls("test000001.xlsx", "Verification")
#申请借款页面换卡验证接口LB0079(借款详情页面换卡调用接口相同LB0079)
...
class testVerification(unittest.TestCase):
  ...
  return nowcode
    ...
    def testverification(self):
        ...
        """
        # set url
        self.url = common.get_url_from_xml('Verification')
    ...
    def checkResult(self):
        ...
        if self.result == '1':
            ...
            self.assertEqual(self.info['result']["retCode"], self.retCode)
            self.assertEqual(self.info['result']["retMessage"], self.retMessage)
            print("调用接口换卡验证失败："+ self.retMessage)