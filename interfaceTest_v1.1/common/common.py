...
def get_xls(xls_name, sheet_name):
    ...
    for i in range(nrows):
        if sheet.row_values(i)[0] != u'case_name':
            if sheet.row_values(i)[0] != "":      #excel中case_name 为空不在向下执行！
...
def get_sql(database_name, table_name, sql_id):
    ...
    return sql
# ****************************** read database.xml ********************************
def get_database_from_xml(name):
    """
    By name get database from database.xml
    :param name: database's database name
    :return: database
    """
    database ={}
    database_path = os.path.join(proDir, 'testFile', 'DATABASE.xml')
    tree = ElementTree.parse(database_path)
    for u in tree.findall('database'):
        url_name = u.get('name')
        if url_name == name:
            for c in u.getchildren():
                database = json.loads(c.text)
    return database
